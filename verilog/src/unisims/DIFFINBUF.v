...
`endif
  ...
  assign O = (OSC_EN_in === 2'b11) ?
      O_OSC_in : (OSC_EN_in === 2'b10 || OSC_EN_in === 2'b01) ? 1'bx : O_out;
  assign O_B = (OSC_EN_in === 2'b11) ?
      O_B_OSC_in : (OSC_EN_in === 2'b10 || OSC_EN_in === 2'b01) ? 1'bx : O_B_out;
...
`endif
  ...
  assign DIFF_TERM_BIN = (DIFF_TERM_REG == "FALSE") ?
      DIFF_TERM_FALSE : (DIFF_TERM_REG == "TRUE") ? DIFF_TERM_TRUE : DIFF_TERM_FALSE;

  assign DQS_BIAS_BIN = (DQS_BIAS_REG == "FALSE") ?
      DQS_BIAS_FALSE : (DQS_BIAS_REG == "TRUE") ? DQS_BIAS_TRUE : DQS_BIAS_FALSE;
...
`endif
  ...
  always @(DIFF_IN_P or DIFF_IN_N or DQS_BIAS_BIN) begin
    if (DIFF_IN_P == 1'b1 && DIFF_IN_N == 1'b0) begin
      O_out <= 1'b1;
      O_B_out <= 1'b0;
    end else if (DIFF_IN_P == 1'b0 && DIFF_IN_N == 1'b1) begin
      O_out <= 1'b0;
      O_B_out <= 1'b1;
    end else if ((DIFF_IN_P === 1'bz || DIFF_IN_P == 1'b0) && (
                 DIFF_IN_N === 1'bz || DIFF_IN_N == 1'b1)) begin
...
`ifdef XIL_TIMING
  reg notifier;
`endif
    specify (DIFF_IN_N => O) = (0: 0: 0, 0: 0: 0); (DIFF_IN_N => O_B) = (0: 0: 0, 0: 0: 0);
    (DIFF_IN_P => O) = (0: 0: 0, 0: 0: 0); (DIFF_IN_P => O_B) = (0: 0: 0, 0: 0: 0);
    (OSC *> O) = (0: 0: 0, 0: 0: 0); (OSC *> O_B) = (0: 0: 0, 0: 0: 0);
    (OSC_EN *> O) = (0: 0: 0, 0: 0: 0); (OSC_EN *> O_B) = (0: 0: 0, 0: 0: 0);
`ifdef XIL_TIMING
