...
`endif
  ...
  always begin
    ...
    end else if (gsr_muxed_sync == 1'b0) begin
      deassign O_bufg_gt;
      deassign clk_count;
      deassign first_rise;
  ...
  always @(posedge I_in, posedge gsr_muxed_sync) begin
    if (gsr_muxed_sync == 1'b1) begin
      ce_sync1 <= 1'b0;
      ce_sync2 <= 1'b0;
    end else begin
  ...
  always @(posedge I_in, negedge clr_inv) begin
    if (~clr_inv) begin
      clr_sync1 <= 1'b0;
      clr_sync2 <= 1'b0;
    end else {clr_sync2, clr_sync1} <= {clr_sync1, 1'b1};
  end
  ...
  always @(i_ce or posedge gsr_muxed_sync or posedge clr_masked) begin
    if (first_toggle_count == 1) begin
      ...
      if (clr_masked == 1'b1 || gsr_muxed_sync == 1'b1) begin
        ...
        first_rise = 1'b1;
      end else if (clr_masked == 1'b0 && gsr_muxed_sync == 1'b0) begin
        if (i_ce == 1'b1 && first_rise == 1'b1) begin
          O_bufg_gt = 1'b1;
          clk_count = 1;
          first_half_period = 1'b1;
          first_rise = 1'b0;
        end else if (clk_count == second_toggle_count && first_half_period == 1'b0) begin
          O_bufg_gt = ~O_bufg_gt;
          clk_count = 1;
          first_half_period = 1'b1;
        end else if (clk_count == first_toggle_count && first_half_period == 1'b1) begin
          O_bufg_gt = ~O_bufg_gt;
          clk_count = 1;
          first_half_period = 1'b0;
        end else if (first_rise == 1'b0) begin
